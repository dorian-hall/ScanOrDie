...
class XCodeBuilder(PlatformBuilder):
	...
	def _CreateCommands(self, arch=None, config='Profile'):
		...
		cmd = [self.compiler, '-project', self.solution, '-configuration', config, 'WWISESDK={}'.format(os.environ['WWISESDK']), 'build']
		cmds.append(cmd)
		return cmds
...
class XCodeDeviceBuilder(XCodeBuilder):
	...
	def _CreateCommands(self, arch=None, config='Profile'):
		...
		for supportedSdk in self.supportedSdks:
			cmd = [self.compiler, '-project', self.solution, '-configuration', config, '-sdk', supportedSdk, 'WWISESDK={}'.format(os.environ['WWISESDK']), 'build']
			cmds.append(cmd)
